...
const SingleItem = ({ item }: SingleItemProps) => {
  ...
  const handleRemoveFromCart = () => {
    console.log("Remove item from cart:", item.id)
    // TODO: Implement remove functionality
  }

  const handleIncreaseQuantity = () => {
    setQuantity(quantity + 1)
    console.log("Increase quantity for item:", item.id, "to", quantity + 1)
  ...
  const handleDecreaseQuantity = () => {
    if (quantity > 1) {
      setQuantity(quantity - 1)
      console.log("Decrease quantity for item:", item.id, "to", quantity - 1)
      // TODO: Implement decrease quantity functionality
    } else {
      return
    }
  }
        ...
        <div className="flex items-center justify-between gap-5">
          <div className="w-full flex items-center gap-5.5">
            <div className="flex items-center justify-center rounded-[5px] bg-gray-2 max-w-[80px] w-full h-17.5">
              <Image width={200} height={200} src={item.imgs?.thumbnails[0] || "/images/products/product-1-bg-1.png"} alt="product" />
            </div>
